...
- versoes root/API/Web/Desktop/SDK JS sincronizadas em 0.9.0.

## Build npm completo

O build semantico completo com `npm ci` nao e declarado como concluido neste ambiente, porque o registry/dependencias de producao nao estao integralmente disponiveis aqui. O `release-win.sh 0.9.0` no servidor oficial de build continua sendo a barreira final antes da publicacao.
