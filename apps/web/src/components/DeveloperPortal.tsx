...
export function DeveloperPortal({ user, onExit }: { user: User; onExit: () => void }) {
  ...
  return <main className="portal-shell developer-portal developer-portal-v2">
    ...
    <section className="portal-main developer-main-v2">
      ...
      {section === "sdk" && <div className="developer-sdk-page"><div className="developer-sdk-grid"><section className="portal-card"><header><TerminalSquare/><div><h2>ginga.py</h2><p>Python e o runtime oficial para bots criados no Ginga. A ergonomia e familiar para quem ja usou discord.py.</p></div></header><div className="sdk-step"><strong>1. Instale o SDK</strong><code>pip install -e ./sdk/python</code></div><div className="sdk-step"><strong>2. Habilite o intent se o bot ler mensagens</strong><span>Em Bots Python, ative <b>Conteudo de mensagens</b> para comandos como !ping.</span></div><div className="sdk-step"><strong>3. Guarde o token no ambiente</strong><code>set GINGA_BOT_TOKEN=seu_token</code></div><div className="sdk-step"><strong>4. Crie o bot</strong><pre><code>{pythonExample}</code></pre></div><button className="secondary-button" onClick={() => void copyText(pythonExample, "Exemplo Python copiado")}><Copy size={16}/> Copiar exemplo</button></section><section className="portal-card"><header><ExternalLink/><div><h2>Gateway e Intents</h2><p>REST para acoes explicitas e Gateway Socket.IO para eventos em tempo real.</p></div></header><ul className="developer-capability-list"><li><strong>Python somente</strong><span>O portal nao publica SDK de bot em JavaScript/TypeScript.</span></li><li><strong>Intents</strong><span>O bot declara GUILDS, GUILD_MESSAGES, MESSAGE_CONTENT e VOICE_STATES conforme a necessidade.</span></li><li><strong>Decorators</strong><span><code>@bot.command</code> sincroniza o catalogo de comandos automaticamente.</span></li><li><strong>Menor privilegio</strong><span>Intents nao substituem as permissoes aprovadas na instalacao nem a ACL do canal.</span></li><li><strong>Reconexao</strong><span>O SDK cuida da sessao do Gateway e separa token de bot da conta humana.</span></li></ul></section></div></div>}
